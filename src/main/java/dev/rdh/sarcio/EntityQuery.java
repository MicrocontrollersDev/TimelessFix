...
import dev.rdh.sarcio.mixin.allocation_rate.ChunkEntityListsAccessor;
import dev.rdh.sarcio.mixin.allocation_rate.ClassInheritanceMultiMapAccessor;
		...
		for (int chunkX = minChunkX; chunkX <= maxChunkX; chunkX++) {
			for (int chunkZ = minChunkZ; chunkZ <= maxChunkZ; chunkZ++) {
				...
				ClassInheritanceMultiMap<Entity>[] entityLists =
					((ChunkEntityListsAccessor) chunk).sarcio$getEntityLists();
				...
				for (int section = minSection; section <= maxSection; section++) {
					List<Entity> entities =
						((ClassInheritanceMultiMapAccessor) entityLists[section]).sarcio$getValues();
